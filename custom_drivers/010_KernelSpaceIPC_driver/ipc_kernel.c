...
#include <linux/io.h>   // Include this header for virt_to_phys()
...
static int ipc_mmap(struct file *filp, struct vm_area_struct *vma) {
    ...
    printk(KERN_INFO "ipc_rw: mmap called. Size: %lu\n", size);

    if (size > SHARED_MEM_SIZE) {
        return -EINVAL;
    }

    // Use virt_to_phys for mapping
    if (remap_pfn_range(vma, vma->vm_start, virt_to_phys(shared_buffer) >> PAGE_SHIFT,
...
static struct file_operations ipc_fops = {
    ...
    .read = ipc_read,
    .write = ipc_write,
    .mmap = ipc_mmap,  // Adding mmap to file operations
};
